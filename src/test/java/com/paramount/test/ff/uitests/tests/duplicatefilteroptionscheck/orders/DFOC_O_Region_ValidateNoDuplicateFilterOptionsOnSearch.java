...
public class DFOC_O_Region_ValidateNoDuplicateFilterOptionsOnSearch extends DuplicateFilterOptionsOrdersTabBaseTest {

    private static final String FILTER_NAME = OrdersLeftFilter.REGION.getDisplayName();

    @Test(priority = 0)
    @Description("Validate narrow search inside Region filter on Orders tab returns exactly one result per option (no duplicates)")
