...
public class DFOC_LI_JobType_ValidateNoDuplicateFilterOptionsOnSearch extends DuplicateFilterOptionsLineItemsTabBaseTest {

    private static final String FILTER_NAME = LineItemsLeftFilter.JOB_TYPE.getDisplayName();

    @Test(priority = 0)
    @Description("Validate narrow search inside Job type filter on Line items tab returns exactly one result per option (no duplicates)")
