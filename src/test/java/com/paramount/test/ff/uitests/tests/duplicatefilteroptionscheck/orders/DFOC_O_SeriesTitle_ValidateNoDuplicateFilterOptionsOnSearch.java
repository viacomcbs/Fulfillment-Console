...
public class DFOC_O_SeriesTitle_ValidateNoDuplicateFilterOptionsOnSearch extends DuplicateFilterOptionsOrdersTabBaseTest {

    private static final String FILTER_NAME = OrdersLeftFilter.SERIES_TITLE.getDisplayName();

    @Test(priority = 0)
    @Description("Validate narrow search inside Series title filter on Orders tab returns exactly one result per option (no duplicates)")
