...
public class DFOC_LI_LineItemStatus_ValidateNoDuplicateFilterOptionsOnSearch extends DuplicateFilterOptionsLineItemsTabBaseTest {

    private static final String FILTER_NAME = LineItemsLeftFilter.LINE_ITEM_STATUS.getDisplayName();

    @Test(priority = 0)
    @Description("Validate narrow search inside Line Item Status filter on Line items tab returns exactly one result per option (no duplicates)")
