...
public class DFOC_O_SystemName_ValidateNoDuplicateFilterOptionsOnSearch extends DuplicateFilterOptionsOrdersTabBaseTest {

    private static final String FILTER_NAME = OrdersLeftFilter.SYSTEM_NAME.getDisplayName();

    @Test(priority = 0)
    @Description("Validate narrow search inside System Name filter on Orders tab returns exactly one result per option (no duplicates)")
