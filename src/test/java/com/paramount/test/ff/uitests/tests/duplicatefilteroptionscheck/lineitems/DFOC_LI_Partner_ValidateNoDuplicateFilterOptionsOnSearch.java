...
public class DFOC_LI_Partner_ValidateNoDuplicateFilterOptionsOnSearch extends DuplicateFilterOptionsLineItemsTabBaseTest {

    private static final String FILTER_NAME = LineItemsLeftFilter.PARTNER.getDisplayName();

    @Test(priority = 0)
    @Description("Validate narrow search inside Partner filter on Line items tab returns exactly one result per option (no duplicates)")
