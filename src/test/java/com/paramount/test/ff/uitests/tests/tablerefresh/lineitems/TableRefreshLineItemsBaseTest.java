...
/** Base setup for BSD-29582 Line Items tab table refresh tests — shared session. */
public abstract class TableRefreshLineItemsBaseTest extends BaseTest {
    ...
    public TableRefreshLineItemsBaseTest() {
    ...
    public void tableRefreshLineItemsSetup() throws InterruptedException {
        login = new Login();
        leftFilterPanelUtil = new LeftFilterPanelUtil();
        tableRefreshUtil = new TableRefreshUtil();
        softAssert = new SoftAssert("tableRefreshLineItemsSetup", getClass().getSimpleName());

        if (!LeftFilterSessionHelper.isLoggedIn()) {
            Logger.logReportMessage("Launching Fulfillment Console for table refresh (Line Items) tests");
            ...
            Logger.logReportMessage("Refreshing Fulfillment Console for next table refresh (Line Items) test");
