...
public class DFOC_O_SubmittedBy_ValidateNoDuplicateFilterOptionsOnSearch extends DuplicateFilterOptionsOrdersTabBaseTest {

    private static final String FILTER_NAME = OrdersLeftFilter.SUBMITTED_BY.getDisplayName();

    @Test(priority = 0)
    @Description("Validate narrow search inside Submitted by filter on Orders tab returns exactly one result per option (no duplicates)")
